...
export async function POST(request: Request) {
  ...
  if (!text || text.length < 50)
    return NextResponse.json({ error: "Text too short. Paste at least a few paragraphs." }, { status: 400 });
  if (text.length > 15000)
    return NextResponse.json({ error: "Text too long. Keep it under 15,000 characters." }, { status: 400 });
